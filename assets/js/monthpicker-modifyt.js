...
$(function () {
  var Monthpicker = (function () {
    ...
    c.prototype.InitValue = function () {
      ...
      // Update matching pattern and splitting logic to YYYY-MM
      this.original_input.value.match("[0-9]{4}-[0-9]{1,2}") &&
        ((b = this.original_input.value.split("-")),
          (this.selectedYear = parseInt(b[0])),
          (this.selectedMonth = parseInt(b[1])),
          (b = !0));

      this.opts.allowNull ||
        b ||
        ((this.selectedMonth = a.getMonth() + 1), // getMonth returns 0-11, so we add 1
          (this.selectedYear = a.getFullYear()),
          this.EnforceBounds());
    };
    ...
    c.prototype.EvaluateOptions = function () {
      ...
      if (null !== this.opts.minValue || null !== this.opts.minYear) {
        if (null !== this.opts.minValue) {
          ...
          a.min.year = parseInt(b[0]);
          a.min.month = parseInt(b[1]);
        } else {
          a.min.year = parseInt(this.opts.minYear);
          a.min.month = 1;
        }
      }

      if (null !== this.opts.maxValue || null !== this.opts.maxYear) {
        if (null !== this.opts.maxValue) {
          b = this.opts.maxValue.split("-"); // Change separator to "-"
          a.max.year = parseInt(b[0]);
          a.max.month = parseInt(b[1]);
        } else {
          a.max.year = parseInt(this.opts.maxYear);
          a.max.month = 12;
        }
      }
    ...
    c.prototype.RefreshInputs = function () {
      if (this.selectedYear && this.selectedMonth) {
        ...
        // Display the selected month in the input field
        this.input.innerHTML =
          this.opts.monthLabels[this.selectedMonth - 1] + " " + this.selectedYear;
      } else {
        this.input.innerHTML =
          '<span class="placeholder">' + this.original_input.placeholder + "</span>";
      }
    };

    c.prototype.EnforceBounds = function () {
      // Enforce minimum year/month bounds
      if (this.bounds.min.year !== null && this.selectedYear < this.bounds.min.year) {
        this.selectedYear = this.bounds.min.year;
        this.selectedMonth = this.bounds.min.month || 1;
      }

      // Enforce maximum year/month bounds
      if (this.bounds.max.year !== null && this.selectedYear > this.bounds.max.year) {
        this.selectedYear = this.bounds.max.year;
        this.selectedMonth = this.bounds.max.month || 12;
      }
    };
    ...
    c.prototype.InitIU = function () {
      ...
      this.original_input.style.display = "block";
    ...
    c.defaultOpts = {
      ...
      maxValue: null,
      maxYear: null,
      monthLabels: "1월 2월 3월 4월 5월 6월 7월 8월 9월 10월 11월 12월".split(" "),
      onSelect: null,
      onClose: null,
      allowNull: !0,
    };
